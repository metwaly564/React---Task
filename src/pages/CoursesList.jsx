...
const CoursesList = () => {
  ...
  const loadCourses = useCallback(async (page = 1, currentSearchTerm = searchTerm, currentCategory = selectedCategory) => {
    ...
    return (
      <div className="max-w-4xl mx-auto animate-fade-in">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-8 text-center my-8 animate-scale-in">
          ...
          <button
            onClick={() => loadCourses(currentPage, searchTerm, selectedCategory)}
      ...
      {courses.length > 0 && (
        <div className="mt-6 text-center animate-scale-in">
          <div className="inline-flex items-center px-4 py-2 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
            ...
            Page {currentPage} of {totalPages} • Showing {courses.length} course{courses.length !== 1 ? 's' : ''}
          </div>
        </div>
      )}
    </div>
